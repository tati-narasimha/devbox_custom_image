...
if (-not $skipGalleryCreation) {
    ...
    New-AzGalleryImageDefinition -GalleryName $galleryName -ResourceGroupName $resourceGroup -Location $location -Name $imageDefName -OsState generalized -OsType Windows -Publisher $companyName -Offer $imageOfferName -Sku $imageSkuName -Feature $features -HyperVGeneration "V2"
}
...
New-AzResourceGroupDeployment -ResourceGroupName $resourceGroup -TemplateFile $temptemplateFilePath -Api-Version "2020-02-14" -imageTemplateName $imageDefName -svclocation $location
Invoke-AzResourceAction -ResourceName $imageDefName -ResourceGroupName $resourceGroup -ResourceType Microsoft.VirtualMachineImages/imageTemplates -ApiVersion "2020-02-14" -Action Run
...
do {
    $status = Get-AzImageBuilderTemplate -ImageTemplateName $imageDefName -ResourceGroupName $resourceGroup | Select-Object -ExpandProperty ProvisioningState
